...
def symbolic_expr(memory, expr):
    ...
    if isinstance(expr, ExprVar):
        ...
        # two wrong answer below
        # for instructor's example is confusing
        # memory Dict will be changed only by StmtAssign

        # # if expr is atomic (memory.args) return value in Dict
        # # e.g.  expr = {ExprVar} a
        # if expr.var in memory.args:
        #     return memory.symbolic_memory[expr.var]
        # # else recursion value of expr.var in Dict
        # # e.g.  expr = {ExprVar} n
        # else:
        #     return symbolic_expr(memory, memory.symbolic_memory[expr.var])

        # another way don't use memory.args judge:
        # val = memory.symbolic_memory[expr.var]  # value in Dict
        # if isinstance(val, ExprBop):
        #     if str(val.left) == str(expr) or str(val.right) == str(expr):
        #         # stop recursion
        #         # must use str() == str()
        #         return ExprBop(val.left, val.right, val.bop)
        #     else:
        #         return ExprBop(symbolic_expr(memory, val.left), symbolic_expr(memory, val.right), val.bop)
        # else:
        #     return val
...
def symbolic_stmt(memory, stmt, rest_stmts, results):
    ...
    if isinstance(stmt, StmtIf):
        ...
        # one of them will symbolic execute then_stmts and rest_stmts
        # other one will symbolic execute else_stmts and rest_stmts
        # also, start them and wait them to join

        p1 = mp.Process(target=symbolic_stmts, args=(memory, stmt.then_stmts + rest_stmts, results, stmt.expr))
        p2 = mp.Process(target=symbolic_stmts, args=(memory, stmt.else_stmts + rest_stmts, results, neg_exp(stmt.expr)))
        p1.start()
        p2.start()
        p1.join()
        p2.join()
...
def expr_2_z3(expr):
    ...
    # Z3 constraints. it will be used by check_cond function which you
    # need to read.
    #
    # Your code here：

    # raise Todo("exercise 7: please fill in the missing code.")
    # like concrete.py fuc: def interpret_expr(memory, expr):
    if isinstance(expr, ExprNum):
        return expr.num
    if isinstance(expr, ExprVar):
        return Int(expr.var)
