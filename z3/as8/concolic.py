...
def concolic_stmt(memory, stmt):
    if isinstance(stmt, StmtAssign):
        ...
        # concrete execute on if-statement, but it needs to store the path condition to the memory.
        #
        # Your code here：
        ...
        # use concrete to control loop
        while interpret_expr(memory, stmt.expr):
            memory.path_condition.append(symbolic_expr(memory, stmt.expr))
            concolic_stmts(memory, stmt.stmts)

        # save the last loop judge expr that not fit while expression
        # neg_exp
...
def concolic_executor(func, init_params, try_times):
    ...
    # and use z3 to generate an input to do next concolic execution
