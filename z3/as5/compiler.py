...
def compile_func(func: calc.Function) -> tac.Function:
    ...
    def compile_expr(expr):
        ...
        if isinstance(expr, calc.ExprBop):
            new_left = compile_expr(expr.left)
            new_right = compile_expr(expr.right)
            if expr.bop is calc.BOp.ADD:
                new_var = next(fresh_var)  # first recursion then next(fresh)
                tac_stmts.append(tac.StmtAssignAdd(new_var, new_left, new_right))
                return new_var
            if expr.bop is calc.BOp.MUL:
                new_var = next(fresh_var)
                tac_stmts.append(tac.StmtAssignMul(new_var, new_left, new_right))
                return new_var
            if expr.bop is calc.BOp.DIV:
                new_var = next(fresh_var)
                tac_stmts.append(tac.StmtAssignDiv(new_var, new_left, new_right))
                return new_var
            if expr.bop is calc.BOp.SUB:
                new_var = next(fresh_var)
                tac_stmts.append(tac.StmtAssignSub(new_var, new_left, new_right))
                return new_var
...
def translation_validation(original_func: calc.Function, result_func: tac.Function):
    ...
    F2 = (Const(original_func_ssa.ret, DeclareSort('S')) == Const(result_func_ssa.ret, DeclareSort('S')))
    # F2 = (calc.gen_cons_stm(calc.ExprVar(original_func_ssa.ret)) == calc.gen_cons_stm(
    #     calc.ExprVar(result_func_ssa.ret)))
    F = Implies(F1, F2)
    solver.add(Not(F))
